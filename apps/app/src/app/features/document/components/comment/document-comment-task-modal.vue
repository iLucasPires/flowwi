...
import type { iDocument, iDocumentComment } from '@/app/features/document/types'
...
const props = defineProps<{
  document: iDocument
  comments: (iDocumentComment & { kindTag: string })[]
}>()
...
async function generate() {
  return apiFetch<{ title: string; description: string }>(
    API_DOCUMENT_URLS.GENERATE_TASK(props.document.id),
    {
      method: 'POST',
...
async function confirmSubtask() {
  if (!selectedIds.value.length || !props.document.task) return
  submitting.value = true
  try {
    const suggestion = await generate()
    await createSubtask({ task: props.document.task, title: suggestion.title })
